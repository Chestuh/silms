...
@section('content')
<h2 class="mb-4"><i class="bi bi-plus-circle me-2"></i>Create rubric</h2>
<div class="card border-0 shadow-sm">
    <div class="card-body">
        <form method="POST" action="{{ route('instructor.rubrics.store') }}">
            @csrf
            <div class="row gy-3">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <select id="material-select" name="material_id" class="form-select @error('material_id') is-invalid @enderror">
                        ...
                        @foreach($materials as $material)
                            <option value="{{ $material->id }}" data-course-id="{{ $material->course_id }}" {{ old('material_id') == $material->id ? 'selected' : '' }}>{{ $material->title }} ({{ $material->course->code ?? 'Unknown' }})</option>
                        @endforeach
                    </select>
            ...
            <div class="mt-4">
                <div class="d-flex align-items-center justify-content-between mb-2">
                    <div>
                        ...
                        <p class="text-muted small mb-0">Define at least one criterion and assign a weight for each item.</p>
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered align-middle mb-0" id="criteria-table">
                        ...
                        <tbody>
                            @php
                                $criteriaOld = old('criteria', [['description' => '', 'weight' => '']]);
                            @endphp
            ...
            <div class="mt-4">
                ...
                <a href="{{ route('instructor.rubrics.index') }}" class="btn btn-outline-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
