    ...
    <div class="row g-0 flex-nowrap">
        ...
        <div class="col-md-4 border-end chat-list-wrap">
            <div class="p-2 border-bottom bg-light">
                <a href="{{ route('student.messages.create') }}" class="btn btn-primary btn-sm w-100"><i class="bi bi-plus-lg me-1"></i>New message</a>
            </div>
            <div class="chat-list list-group list-group-flush overflow-auto">
                @forelse($conversations as $c)
                    <a href="{{ route('student.messages.thread', ['user' => $c['user']->id]) }}" class="list-group-item list-group-item-action chat-list-item {{ isset($user) && $user->id === $c['user']->id ? 'active' : '' }}">
                ...
                @empty
                    <div class="list-group-item text-muted text-center py-4">No conversations yet.<br><a href="{{ route('student.messages.create') }}">Start a message</a></div>
                @endforelse
            </div>
        </div>
        {{-- Thread or placeholder --}}
        <div class="col-md-8 d-flex flex-column chat-thread-wrap">
            @if(isset($user))
                ...
                <div class="chat-input border-top p-2 bg-light">
                    <form method="POST" action="{{ route('student.messages.store') }}" class="d-flex gap-2 align-items-end">
                        @csrf
            ...
            @else
                <div class="flex-grow-1 d-flex align-items-center justify-content-center text-muted p-4">
                    <div class="text-center">
                        ...
                        <p class="mt-2 mb-0">Select a conversation or <a href="{{ route('student.messages.create') }}">start a new message</a></p>
                    </div>
                </div>
            @endif
        </div>
